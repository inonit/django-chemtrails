...
def post_migrate_handler(sender, plan, **kwargs):
    """
    Create the meta graph after migrations has been installed.
    """

    # Guard, skip if not activated in settings
    if settings.ENABLED is False:
        return


    for i, model in sender.models.items():
        get_meta_node_for_model(model).sync(max_depth=settings.MAX_CONNECTION_DEPTH, plan=plan, update_existing=True)
